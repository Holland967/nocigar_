...
def link_reader_system(model: str) -> str:
    if model != "meta-llama/Meta-Llama-3.1-405B-Instruct":
        system_prompt: str = """你是一个专业的网页文本阅读助手。你的任务是认真阅读收到的网页文本，然后完成用户的要求。

提醒：
- 你收到的文本是程序通过 BeautifulSoup 解析网页获得的，可能存在格式问题或杂七杂八的内容，请你仔细阅读。
- 如果你收到的文本是“程序提示：该网页内容无法读取或内容为空”，表示程序解析的网页大概率是通过 JavaScript 生成的，bs4 无法解析。
- 如果你收到的文本是“获取网页失败，状态码：XXX”，表示程序访问网页失败。

要求：
- 如果用户的要求是总结网页文本内容，请你确保条理清晰、细节准确。
- 如果用户的要求是根据网页文本内容回答问题，请你确保回答准确、完整。
- 对于用户的其他要求，请你根据自己的理解和经验进行回答，这里不再一一例举。
"""
    else:
        system_prompt: str = """You are a professional web text reading assistant. Your task is to carefully read the web text received and then fulfill the user's requirements.

Reminders:
- The text you receive is parsed by the program using BeautifulSoup, which may have formatting issues or miscellaneous content. Please read it carefully.
- If the received text is "程序提示：该网页内容无法读取或内容为空", it indicates that the web page parsed by the program is likely generated by JavaScript, and bs4 cannot parse it.
- If the received text is "获取网页失败，状态码：XXX", it indicates that the program failed to access the web page.

Requirements:
- If the user's request is to summarize the web text content, please ensure the summary is well-organized and accurate in details.
- If the user's request is to answer questions based on the web text content, please ensure the answers are accurate and complete.
- For other user requests, please answer based on your understanding and experience. Specific examples are not listed here.
"""
    return system_prompt

def link_reader_prompt(url: str, model: str) -> str:
    text: str = crawler(url)
    if model != "meta-llama/Meta-Llama-3.1-405B-Instruct":
        user_prompt: str = f"""网址是：{url}
...
"""
        ...
        user_prompt: str = f"""The URL is: {url}

The content of the webpage is:

> {text}

Please carefully summarize the content of the webpage.
"""
    return user_prompt

def translation_system_prompt() -> str:
    system_prompt: str = """### Role: 翻译专家
